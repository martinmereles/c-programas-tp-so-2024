...
void sys_thread_cancel(int tid_cancel, int ppid, int tid)
{
    ...
    if (tcb_encontrado_exec != NULL)
    {
        finalizar_hilo(tcb_encontrado_exec->ppid, tid_cancel; QUEUE_EXEC);
    }
    else if (tcb_encontrado_blocked != NULL)
    {
...
void sys_mutex_lock(char *nombre, int pid, int tid)
{
    ...
    if (mutex_encontrado != NULL){
        ...
        list_add(QUEUE_BLOCKED, tcb_encontrado);
        list_add(mutex_encontrado->bloqueados, tcb_encontrado);
